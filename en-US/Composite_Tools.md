...
There is a python script named ```psfbuildcomp``` found in the [Pysilfont][pysilfont] collection. It accepts a list of composite definitions and constructs a composite glyph for each definition line. For example,

```psfbuildcomp -i composites.txt -r V CharisSIL-Regular.ufo ```
